...
// 2. Tarik data lama menggunakan 'item_id'
...
$db_category = isset($item['category']) ? $item['category'] : '';
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .main-content h2 { margin-bottom: 40px; font-weight: 800; color: #2d3748; }
        ...
        .form-row input:focus, .form-row textarea:focus, .form-row select:focus { border-color: #00a896; background-color: #fff; }
        ...
        <div class="form-card">
            <form id="editReportForm" action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    ...
                    <input type="text" name="item_name" value="<?php echo isset($item['item_name']) ? htmlspecialchars($item['item_name']) : ''; ?>" required>
                </div>

                <div class="form-row">
                    <label>Category :</label>
                    <select name="category" required>
                        <option value="Electronics" <?php if($db_category == 'Electronics') echo 'selected'; ?>>Electronics</option>
                        <option value="Documents" <?php if($db_category == 'Documents') echo 'selected'; ?>>Documents</option>
                        <option value="Keys" <?php if($db_category == 'Keys') echo 'selected'; ?>>Keys</option>
                        <option value="Bags" <?php if($db_category == 'Bags') echo 'selected'; ?>>Bags</option>
                        <option value="Wallets" <?php if($db_category == 'Wallets') echo 'selected'; ?>>Wallets</option>
                        <option value="Personal Belongings" <?php if($db_category == 'Personal Belongings') echo 'selected'; ?>>Personal Belongings</option>
                        <option value="Others" <?php if($db_category == 'Others') echo 'selected'; ?>>Others</option>
                    </select>
                </div>

                <div class="form-row">
                    <label>Location Found :</label>
                    <input type="text" name="location" value="<?php echo isset($item['location']) ? htmlspecialchars($item['location']) : ''; ?>" required>
                </div>

                <div class="form-row">
                    <label>Date Found :</label>
                    <input type="date" name="item_date" id="item_date" max="<?php echo $today; ?>" value="<?php echo isset($item['item_date']) ? $item['item_date'] : ''; ?>" required>
                </div>

                <div class="form-row">
                    <label>Item Description :</label>
                    <textarea name="description" required><?php echo isset($item['description']) ? htmlspecialchars($item['description']) : ''; ?></textarea>
                </div>

                <div class="form-row">
                    <label>Phone Number :</label>
                    <input type="text" name="phone_number" value="<?php echo isset($item['phone_number']) ? htmlspecialchars($item['phone_number']) : ''; ?>" required>
                </div>

                <div class="form-row">
                    <label>Current Photo :</label>
                    <?php if(!empty($item['image_path']) && file_exists("uploads/" . $item['image_path'])): ?>
                        <img src="uploads/<?php echo $item['image_path']; ?>" style="max-height: 100px; border-radius: 5px;">
                    <?php else: ?>
                        <span style="color: #a0aec0; font-size: 14px;">No Image Uploaded</span>
                    <?php endif; ?>
                </div>
